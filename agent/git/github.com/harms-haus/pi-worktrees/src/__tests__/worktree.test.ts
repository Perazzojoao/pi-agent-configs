...
function mockCwdChangeSuccess(events: { emit: ReturnType<typeof vi.fn> }, cwd: string): void {
  getCurrentWorktreePath.mockReturnValue(cwd);
  events.emit.mockImplementation((_channel: string, payload: unknown) => {
    (payload as { resolve: (result: { ok: true; cwd: string }) => void }).resolve({
      ok: true,
      cwd,
    });
  });
...
describe("switchCwd", () => {
  ...
  it("emits pi-cwd change event with target path", async () => {
    const { api, events, sendMessage } = createMockAPI();
    const ctx = createMockContext();
    const targetPath = FEATURE_PATH;
    mockCwdChangeSuccess(events, targetPath);
    ...
    await switchCwd(api, ctx, targetPath);

    expect(events.emit).toHaveBeenCalledWith(
      "pi-cwd:change",
      expect.objectContaining({ path: targetPath, ctx }),
    );
    expect(sendMessage).not.toHaveBeenCalled();
  });

  it("sets current worktree path after successful cwd change", async () => {
  ...
  it("appends entry with all fields including defaultBranch", async () => {
    const { api, appendEntry, events } = createMockAPI();
    const ctx = createMockContext();
    const targetPath = FEATURE_PATH;
    mockCwdChangeSuccess(events, targetPath);
  ...
  it("does not emit cwd change when target path does not exist", async () => {
    const { api, appendEntry, events } = createMockAPI();
    const ctx = createMockContext();
    vi.mocked(existsSync).mockReturnValue(false);

    await expect(switchCwd(api, ctx, FEATURE_PATH)).rejects.toThrow(
      "Target path does not exist: " + FEATURE_PATH,
    );

    expect(events.emit).not.toHaveBeenCalled();
    expect(setCurrentWorktreePath).not.toHaveBeenCalled();
    expect(appendEntry).not.toHaveBeenCalled();
  });

  it("does not update state when cwd change fails", async () => {
    const { api, appendEntry, events } = createMockAPI();
    const ctx = createMockContext();
    events.emit.mockImplementation((_channel: string, payload: unknown) => {
      (payload as { resolve: (result: { ok: false; error: string }) => void }).resolve({
        ok: false,
        error: "boom",
      });
    });

    await expect(switchCwd(api, ctx, FEATURE_PATH)).rejects.toThrow("boom");

    expect(setCurrentWorktreePath).not.toHaveBeenCalled();
    expect(appendEntry).not.toHaveBeenCalled();
  });

  it("fails without updating state when cwd integration times out", async () => {
    vi.useFakeTimers();
    try {
      const { api, appendEntry } = createMockAPI();
      const ctx = createMockContext();

      const promise = switchCwd(api, ctx, FEATURE_PATH);
      const assertion = expect(promise).rejects.toThrow("CWD integration unavailable or timed out");
  ...
  it("ignores late cwd integration responses after timeout", async () => {
    vi.useFakeTimers();
    try {
      const { api, appendEntry, events } = createMockAPI();
      const ctx = createMockContext();
      let respond: ((result: { ok: true; cwd: string }) => void) | undefined;
      events.emit.mockImplementation((_channel: string, payload: unknown) => {
        respond = (payload as { resolve: (result: { ok: true; cwd: string }) => void }).resolve;
      });
      ...
      respond?.({ ok: true, cwd: FEATURE_PATH });
      expect(setCurrentWorktreePath).not.toHaveBeenCalled();
      expect(appendEntry).not.toHaveBeenCalled();
    } finally {
      vi.useRealTimers();
