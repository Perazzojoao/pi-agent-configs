...
describe("handleWtCleanup", () => {
  ...
  it("switch CWD failure after removing current worktree keeps state on default branch", async () => {
    const pi = createMockAPI().api;
    const ctx = createMockContext();
    vi.mocked(getCurrentBranch).mockReturnValue(FEATURE_BRANCH);
    vi.mocked(switchCwd).mockRejectedValueOnce(new Error("cwd unavailable"));
    ...
    expect(setCurrentBranch).toHaveBeenCalledWith(getDefaultBranch());
    expect(setCurrentBranch).not.toHaveBeenCalledWith(FEATURE_BRANCH);
    expect(setCurrentWorktreePath).toHaveBeenCalledWith(MAIN_REPO);
    expect(persistWorktreeState).toHaveBeenCalledWith(pi);
    expect(updateFooterStatus).toHaveBeenCalledWith(ctx);
    expect(ctx.ui.notify).toHaveBeenCalledWith(
      "Cleaned up worktree but failed to switch CWD. State was reset to main: cwd unavailable",
      "error",
    );
