...
describe("handleWtMerge", () => {
  ...
  it("switch CWD failure after removing merged worktree keeps state on main branch", async () => {
    const pi = createMockAPI().api;
    const ctx = createMockContext();
    ...
    expect(updateFooterStatus).toHaveBeenCalledWith(ctx);
    expect(ctx.ui.notify).toHaveBeenCalledWith(
      "Merged but failed to switch CWD. State was reset to main: cwd unavailable",
      "error",
    );
