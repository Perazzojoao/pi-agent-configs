...
export function restoreWorktreeFromBranch(ctx: ExtensionContext, pi?: ExtensionAPI): void {
  ...
  for (let i = entries.length - 1; i >= 0; i--) {
    ...
    if (
      ...
      if (!isExistingDirectory(data.mainRepoPath)) continue;
      ...
      if (data.currentWorktreePath && isExistingDirectory(data.currentWorktreePath)) {
