...
function newSgrState(): SgrState {
	return { fg: false, bg: false, intensity: false, italic: false, underline: false, blink: false, reverse: false, conceal: false, strike: false };
}
	...
	for (let i = 0; i < params.length; i++) {
		...
		if (code === 0) {
			...
			if (mode === 5 && Number.isFinite(params[i + 2])) {
				...
				i += 2;
			} else if (mode === 2 && Number.isFinite(params[i + 2]) && Number.isFinite(params[i + 3]) && Number.isFinite(params[i + 4])) {
				if (isBackground) state.bg = true;
				else state.fg = true;
				i += 4;
...
function truncateAnsiToWidthInternal(value: string, maxWidth: number, closeBackground: boolean): { text: string; truncated: boolean } {
