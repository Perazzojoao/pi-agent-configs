 ...
 * - /tilldone-status:  interactive overlay with full task details
 * - /tilldone: activate strict mode (mandatory task workflow)
 *
...
export default function (pi: ExtensionAPI) {
	...
	let tilldoneUsedInBranch = false
			...
			return {
				...
				render(width: number): string[] {
					...
					const rows = visible.map(t => {
						const icon =
							t.status === 'done' ? theme.fg('success', STATUS_ICON.done) : theme.fg('accent', STATUS_ICON.inprogress)
	...
	pi.on('tool_call', async (event, _ctx) => {
		if (!strictModeEnabled || event.toolName === 'tilldone') return { block: false }
		...
		if (tasks.length === 0) {
			return {
				block: true,
				reason:
					'🚫 TillDone strict mode is active and no tasks are defined. Use `tilldone new-list` or `tilldone add` before using other tools.',
			}
		}
		if (pending.length === 0) {
			return {
				block: true,
				reason:
					'🚫 TillDone strict mode is active and all tasks are done. Use `tilldone add` for more tasks or `tilldone new-list` to start fresh.',
			}
		}
		if (active.length === 0) {
			return {
	...
	pi.on('agent_end', async (_event, _ctx) => {
		if (!strictModeEnabled && !tilldoneUsedInBranch) return
		...
		const content = strictModeEnabled
			? `⚠️ You still have ${incomplete.length} incomplete task(s):\n\n${taskList}\n\nStrict mode is active: continue working on them or mark them done with \`tilldone toggle\`.`
			: `ℹ️ You still have ${incomplete.length} incomplete task(s):\n\n${taskList}\n\nIf useful, continue working on them or update their status with \`tilldone toggle\`.`
		...
		pi.sendMessage(
			...
			strictModeEnabled ? { triggerTurn: true } : undefined,
		)
	})
	...
	pi.registerTool({
		...
		label: 'TillDone',
		description:
			'Manage your task list. By default this tool is optional; only /tilldone enables strict mandatory workflow for other tools. ' +
			...
			'Typical usage: mark a task inprogress while working, then done when finished. ' +
			'Use new-list to start a themed list with a title and description; use clear/new-list when context changes.',
	...
	pi.registerCommand('tilldone', {
		description: 'Activate TillDone strict mode (mandatory task workflow)',
		handler: async (_args, ctx) => {
			strictModeEnabled = true
			nudgedThisCycle = false

			ctx.ui.notify('TillDone strict mode activated: other tools now require active TillDone tasks.', 'info')
			refreshUI(ctx)
		},
	})

	pi.registerCommand('tilldone-off', {
		description: 'Deactivate TillDone strict mode (return to optional workflow)',
		handler: async (_args, ctx) => {
			strictModeEnabled = false
			nudgedThisCycle = false

			ctx.ui.notify('TillDone strict mode deactivated: task workflow is optional again.', 'info')
			refreshUI(ctx)
		},
	})

	pi.registerCommand('tilldone-status', {
		description: 'Show all TillDone tasks on the current branch',
		handler: async (_args, ctx) => {
			if (!ctx.hasUI) {
				ctx.ui.notify('/tilldone-status requires interactive mode', 'error')
				return
			}
	...
	pi.registerShortcut('alt+l', {
		description: 'Toggle TillDone current-task widget',
		handler: async ctx => {
			isWidgetVisible = !isWidgetVisible
			refreshWidget(ctx)
			ctx.ui.notify(`TillDone widget ${isWidgetVisible ? 'shown' : 'hidden'}.`, 'info')
		},
	})
}
