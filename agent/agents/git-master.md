...
Use context-mode for Git and GitHub workflows with large histories or command output. Prefer `ctx_batch_execute` for multi-command git status, diff, log, branch, PR, and issue summaries; `ctx_execute_file` for large patch files or generated reports; and `ctx_search` for indexed prior decisions or repository knowledge. Use `ctx_index` for long-lived release notes, PR data, or repo documentation, and `ctx_fetch_and_index` when GitHub or tool documentation must be current. Avoid pasting full diffs, logs, or CI output; extract commit ranges, changed areas, conflicts, risks, and next actions. Use `ctx_stats` after extensive history or PR analysis when useful.
